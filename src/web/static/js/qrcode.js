/**
 * MeshCore Bridge - Pure Vanilla JS QR Code Generator
 * Renderizador de Códigos QR offline para canales y contactos LoRa.
 * Basado en especificación QR ISO/IEC 18004.
 */

(function (global) {
  "use strict";

  const QRMode = { NUMBER: 1, ALPHA_NUM: 2, 8: 4, KANJI: 8 };
  ...
  QR8bitByte.prototype = {
    getLength: function () {
      return unescape(encodeURIComponent(this.data)).length;
    },
    write: function (buffer) {
      const utf8 = unescape(encodeURIComponent(this.data));
      for (let i = 0; i < utf8.length; i++) {
        buffer.put(utf8.charCodeAt(i), 8);
      }
    },
  };
  ...
  function drawEye(ctx, x, y, size, outerColor, innerColor) {
    const rOuter = size * 0.28;
    const rInner = size * 0.22;
    const strokeW = size / 7;

    // 1. Marco exterior redondeado
    ctx.fillStyle = outerColor;
    drawRoundRect(ctx, x, y, size, size, rOuter);

    // 2. Vaciado interior blanco
    ctx.fillStyle = "#ffffff";
    const cutoutOffset = strokeW;
    const cutoutSize = size - strokeW * 2;
    drawRoundRect(ctx, x + cutoutOffset, y + cutoutOffset, cutoutSize, cutoutSize, rOuter * 0.7);

    // 3. Núcleo central redondeado
    ctx.fillStyle = innerColor || outerColor;
    const dotOffset = strokeW * 2;
    const dotSize = size - strokeW * 4;
    drawRoundRect(ctx, x + dotOffset, y + dotOffset, dotSize, dotSize, rInner);
  }
  ...
  const QRCodeGenerator = {
    renderToCanvas: function (canvas, text, options) {
      ...
      const ecLevel = options.errorCorrection || QRErrorCorrectLevel.Q;
      ...
      // 2. Gradiente moderno para los datos
      const grad = ctx.createLinearGradient(margin, margin, size - margin, size - margin);
      grad.addColorStop(0, "#081b33");
      grad.addColorStop(0.6, "#0c2847");
      grad.addColorStop(1, "#0284c7");

      const eyeColor = "#04385a";
      const eyeCenterColor = "#0284c7";

      // 3. Renderizar ojos estilizados en las 3 esquinas
      const eyeSize = 7 * cell;
      drawEye(ctx, margin, margin, eyeSize, eyeColor, eyeCenterColor);
      drawEye(ctx, margin + (count - 7) * cell, margin, eyeSize, eyeColor, eyeCenterColor);
      drawEye(ctx, margin, margin + (count - 7) * cell, eyeSize, eyeColor, eyeCenterColor);

      // 4. Renderizar módulos de datos con forma de puntos suaves
      ctx.fillStyle = grad;
      const dotRadius = Math.max(1, cell * 0.35);
      ...
      for (let r = 0; r < count; r++) {
        for (let c = 0; c < count; c++) {
          ...
          if (qr.isDark(r, c)) {
            ...
            const w = cell;
            const h = cell;
            drawRoundRect(ctx, x + cell * 0.05, y + cell * 0.05, w * 0.9, h * 0.9, dotRadius);
          }
        }
      }
    },
    ...
    toDataURL: function (text, size) {
      ...
      this.renderToCanvas(canvas, text, { size: size || 256 });
