...
- **Agentes Participantes**: Agente 0 (Lead Orchestrator), Agente 1 (Firmware Investigator), Agente 2 (Bridge Architect), Agente 5 (Security Auditor).
- **Acciones Realizadas**:
  1. **Auditoría y Gap Analysis (`implementation_plan.md`)**:
     - Elaborado y publicado el informe exhaustivo con matriz de paridad (Sección 1 y 2), brechas pendientes (Sección 3 con 8 grupos de capacidades) y hoja de ruta por fases (Sección 5).
  2. **Especificación del Protocolo (`docs/PROTOCOL_SPEC.md`)**:
     - Clarificada la coexistencia entre el Protocolo Companion primario (`<` / `>`) y el protocolo wire legacy (`0xAA`/`0x55` con CRC-16 CCITT).
     - Completada la tabla de `CommandType` con los códigos `33` a `35` (`SIGN_*`), `40` a `43` (`CUSTOM_VARS`, `TUNING`), `50` (`BINARY_REQ`), `51` (`FACTORY_RESET`), `52` (`PATH_DISCOVERY`), `54` (`SET_FLOOD_SCOPE_KEY`), `55` (`SEND_CONTROL_DATA`), `56` (`GET_STATS`), `57` (`SEND_ANON_REQ`), `58`-`59` (`AUTOADD_CONFIG`), `60` (`ALLOWED_REPEAT_FREQ`), `61` (`PATH_HASH_MODE`), `62` (`CHANNEL_DATA`), `63`-`64` (`DEFAULT_FLOOD_SCOPE`), `65` (`RAW_PACKET`).
  3. **Tipos de Protocolo (`src/protocol_types.py`)**:
     - Añadido `AnonReqType` con `REGIONS (0x01)`, `OWNER (0x02)` y `BASIC (0x03)`.
  4. **Ejecutor Administrativo de Repetidores (`src/admin/repeater_executor.py`)**:
     - Añadido método `_try_execute_binary_or_anon` que despacha solicitudes binarias de vecinos (`req_neighbours_sync`), propietario (`req_owner_sync`), regiones (`req_regions_sync`), reloj (`req_basic_sync`) y permisos (`req_acl_sync`) directamente con el SDK oficial o mediante comandos de respaldo.
  5. **Sincronización y Empaquetado (`/deploy/`)**:
     - Ejecutado `python scripts/sync_deploy.py` para sincronizar paquetes ZIP, tar.gz y sumas SHA256.
...
- **Acciones Realizadas**:
  ...
  2. **Ciclo de Vida de Presencia y Limpieza de Métricas (`contact_manager.py`, `rx_router.py`, `repeater_handler.py`, `nodes.js`)**:
     - `NodeContactInfo.to_dict()`: Implementado cálculo determinista según `diff = now - last_seen`:
       ...
       - `12h - 24h`: `presence_status = "idle"`, `status_label = "Inactivo"`, métricas SNR/RSSI/LQI preservadas.
       - `> 24h`: `presence_status = "offline"`, `status_label = "Desconectado"`, **métricas vaciadas estrictamente** (`last_rssi=None`, `last_snr=None`, `lqi_score=0.0`, `lqi_status="DISCONNECTED"`).
       - Inclusión de `last_seen_iso` y `last_seen_formatted` con fecha y hora local completa.
     - En `rx_router.py` y `repeater_handler.py`: ante la recepción de cualquier chat, telemetría o ACK de un nodo no local, se actualiza `last_seen=time.time()` y se emite `contact_updated` por WebSocket.
     - En `nodes.js`:
...
- **Agentes Participantes**: Agente 0 (Lead Orchestrator), Agente 4 (Web Architect), Agente 2 (Bridge Architect), Agente 5 (Security Auditor).
- **Problema / Requerimiento**:
  - El usuario reportó: (1) Problemas de estilos CSS/HTML en la vista de mensajes de chat; (2) En Contactos, verificar `contacts-toolbar`, asegurar que se muestre una marca de favorito en las cards y comprobar que los filtros funcionen correctamente; (3) En Mapas, los nodos no se estaban posicionando en el mapa y el mapa debe centrarse en la posición del nodo local; (4) Chequeo general del frontend buscando errores y código obsoleto.
- **Causas Raíz Identificadas y Correcciones**:
  1. **Vista de Chat ([`src/web/static/js/modules/chat.js`](file:///c:/Users/Ruby/Desktop/meshcore-bridge/src/web/static/js/modules/chat.js), [`src/web/static/css/app.css`](file:///c:/Users/Ruby/Desktop/meshcore-bridge/src/web/static/css/app.css))**:
     - *Causa*: `chat.js` creaba las burbujas con `<div class="message-bubble">`, mientras que `app.css` únicamente estilizaba `.msg-bubble`, provocando que las burbujas carecieran de fondo, bordes redondeados y gradientes salientes. Además, el botón de compartir ubicación invocaba `this.shareCurrentLocation()` que no estaba definido, y los acuses de recibo no coincidían con los selectores de entrega.
     - *Corrección*: Se actualizó a `<div class="msg-bubble message-bubble">`, se ajustaron los estilos para `.msg-bubble, .message-bubble` y contraste blanco en mensajes salientes. Se implementó `shareCurrentLocation()` con `navigator.geolocation` y fallback a coordenadas de estación local (`localGpsLat`/`localGpsLon`). Se añadió detección de coordenadas GPS para renderizar tarjetas `.chat-location-card` con botón interactivo "Ver en mapa".
  2. **Libreta de Contactos y Favoritos ([`src/web/static/js/modules/nodes.js`](file:///c:/Users/Ruby/Desktop/meshcore-bridge/src/web/static/js/modules/nodes.js), [`src/web/controllers/contacts_controller.py`](file:///c:/Users/Ruby/Desktop/meshcore-bridge/src/web/controllers/contacts_controller.py), [`src/web/static/css/app.css`](file:///c:/Users/Ruby/Desktop/meshcore-bridge/src/web/static/css/app.css))**:
     - *Causa*: Las tarjetas no renderizaban ninguna marca visual de favorito en el DOM, no había botón para alternar el estado y el backend (`contacts_controller.py`) ignoraba `is_favorite` en `_create_or_update_contact`. Al filtrar por favoritos, si no había coincidencias la grilla quedaba en blanco sin feedback.
     - *Corrección*: Se agregó botón estrella interactivo `.btn-toggle-fav` a cada tarjeta con animación de brillo dorado, toggle en tiempo real, persistencia vía `POST /api/contacts` con `{ public_key, is_favorite }`, actualización del contador `#countFavContacts` y mensaje de estado vacío `.contacts-empty-filter-state` cuando no hay coincidencias.
  3. **Mapa y Centrado en Nodo Local ([`src/web/static/js/modules/map.js`](file:///c:/Users/Ruby/Desktop/meshcore-bridge/src/web/static/js/modules/map.js), [`src/web/static/index.html`](file:///c:/Users/Ruby/Desktop/meshcore-bridge/src/web/static/index.html), [`src/web/static/css/app.css`](file:///c:/Users/Ruby/Desktop/meshcore-bridge/src/web/static/css/app.css))**:
     - *Causa*: `nodes.js` emitía `EVENTS.NODE_UPDATED` pasando `null`, provocando que `map.js` ignorara las actualizaciones. El mapa iniciaba con coordenadas fijas hardcodeadas y no existía método de centrado local ni se alimentaba el panel `#mapNodesList`.
     - *Corrección*: `nodes.js` ahora emite la lista completa de nodos deduplicados. En `map.js`, se implementaron `getLocalNodeCoordinates()`, `centerOnLocalNode(zoom, showToast)` y `centerMapOnCoords(lat, lon, zoom)`. Se añadió marcador distintivo con pulso verde `.marker-local-station` para la estación base, se implementó botón `btnCenterLocalNode` en el selector de capas y se alimenta `#mapNodesList` con tarjetas compactas interactivas con paneo al hacer clic.
  4. **Auditoría y Limpieza de Frontend ([`src/web/static/js/app.js`](file:///c:/Users/Ruby/Desktop/meshcore-bridge/src/web/static/js/app.js))**:
     - *Corrección*: Se eliminó la función duplicada `escapeHtml` en `app.js` importándola desde `./core/utils.js`, y se expusieron `centerMapOnCoords` y `centerOnLocalNode` en el contexto global de la aplicación.
- **Módulos Modificados**: `src/web/controllers/contacts_controller.py`, `src/web/static/js/modules/chat.js`, `src/web/static/js/modules/nodes.js`, `src/web/static/js/modules/map.js`, `src/web/static/js/app.js`, `src/web/static/css/app.css`, `src/web/static/index.html`, `deploy/**`.
- **Fecha**: 2026-09-03
